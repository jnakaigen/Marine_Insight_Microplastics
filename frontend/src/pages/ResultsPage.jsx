   ...
   const handleDownloadPDF = () => {
    ...
    useEffect(() => {
        if (!data && urlId) {
            ...
            axios.get(API(`/batch/${urlId}/`), {
                headers: token ? { Authorization: `Bearer ${token}` } : {}
            })
                .then(res => setData(res.data))
                .catch(err => {
                    console.error("API Error:", err);
                    setData({ error: err.response?.data?.error || 'Unable to load results.' });
                })
        ...
        return (
            <div className="theme-wrapper" style={{ minHeight: '100vh' }}>
                ...
                <div style={{ textAlign: 'center', padding: '100px 20px', color: 'var(--text-primary)' }}>
                    ...
                    <p>{data?.error || 'Open a batch from the dashboard or upload a new set of images to view results here.'}</p>
                </div>
            </div>
        );
    }
